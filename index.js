...
Using USApop, find the total population from the zoos array using the .reduce()
method. Remember, the reduce method takes two arguments: a callback (which 
itself takes two args - the accumulator and the item), and an initial value for
the count. */
...
cuboid's volume or surface area. Cuboids are similar to cubes but do not have 
...
Create a method called volume using CuboidMaker's prototype that returns the
volume of a given cuboid's length, width, and height. Formula for cuboid 
...
Create another method called surfaceArea using CuboidMaker's prototype that 
returns the surface area of a given cuboid's length, width, and height. Formula 
for cuboid surface area of a cube: 
